...
def plot(num1, num2, y1, y2, save_dir):
    ...
    plt.scatter(x, y1, marker="s", s=60, edgecolors='k', linewidth=2)
    plt.scatter(x, y2, marker="^", s=60, edgecolors='b', linewidth=2)
...
def main(args):
    ...
    data1 = parse_file(os.path.join(root, f'build/test_kernel_{args.one}.txt'))
    data2 = parse_file(os.path.join(root, f'build/test_kernel_{args.another}.txt'))
...
def parse_args():
    ...
    parser.add_argument('--save_dir', default='build/')
